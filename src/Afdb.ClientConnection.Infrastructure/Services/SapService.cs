...
internal sealed class SapService : ISapService
{
    ...
    public async Task<List<SapProjectData>> GetProjectsByCountryAsync(string countryCode, 
    ...
    public async Task<List<SapProjectLoanNumber>> GetProjectLoanNumbersAsync(string sapCode, 
        CancellationToken cancellationToken = default)
    {
        var baseUrl = _configuration["Sap:ProjectLoanNumberUrl"];
        if (string.IsNullOrEmpty(baseUrl))
            throw new InvalidOperationException("ERR.Sap.ProjectLoanNumberUrlMissing");
        ...
        List<SapProjectLoanNumber> projectLoans = JsonSerializer.Deserialize<List<SapProjectLoanNumber>>(content, options) ?? [];
